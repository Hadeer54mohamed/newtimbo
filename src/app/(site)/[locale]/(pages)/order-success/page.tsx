...
const OrderSuccessPage = () => {
  ...
  const handleSendWhatsApp = () => {
    if (!order) return;
    try {
      const orderDetails = convertOrderToOrderDetails(order, order.order_items || []);
  ...
  if (loading) {
    return (
      <>
        ...
        />
        <section className="py-20 bg-gray-2 flex justify-center">
          ...
          <div className="bg-white shadow-1 rounded-[10px] p-8">
            ...
            <div className="text-center mb-8">
              ...
              <h1 className="text-3xl font-bold text-dark mb-2">
                {locale === "ar" ? "تم إرسال طلبك بنجاح!" : "Order Placed Successfully!"}
              </h1>
            ...
            <div className="bg-white rounded-xl p-8 mb-8 shadow-sm text-center">
              <h3 className="text-lg font-semibold text-gray-800 mb-4">
                {locale === "ar" ? "حالة الطلب الحالية" : "Current Order Status"}
              </h3>
              ...
              <p className="text-gray-600">
                ...
                {order.status === "cancelled" &&
                  (locale === "ar" ? "تم إلغاء طلبك" : "Your order has been cancelled")}
              </p>
            </div>

            {/* Order Details & Customer Info */}
            <div className="border-t border-gray-3 pt-8 grid grid-cols-1 lg:grid-cols-2 gap-8">
              {/* Order Info */}
              <div>
                ...
                <div className="space-y-3">
                  <div className="flex justify-between">
                    <span className="text-dark-5">{locale === "ar" ? "رقم الطلب:" : "Order ID:"}</span>
                    <span className="font-medium text-dark">{order.id}</span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-dark-5">{locale === "ar" ? "تاريخ الطلب:" : "Order Date:"}</span>
                    <span className="font-medium text-dark">{new Date(order.created_at).toLocaleDateString()}</span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-dark-5">{locale === "ar" ? "حالة الطلب:" : "Status:"}</span>
              ...
              {(order.customer_first_name || order.customer_last_name) && (
                <div>
                  <h3 className="text-xl font-semibold text-dark mb-4">{locale === "ar" ? "معلومات العميل" : "Customer Information"}</h3>
                  <div className="space-y-3">
                    {(order.customer_first_name || order.customer_last_name) && (
                      <div>
                        <span className="text-dark-5 block">{locale === "ar" ? "الاسم:" : "Name:"}</span>
                        <span className="font-medium text-dark">{order.customer_first_name} {order.customer_last_name}</span>
                      </div>
                    )}
                    {order.customer_phone && (
                      <div>
                        <span className="text-dark-5 block">{locale === "ar" ? "رقم الهاتف:" : "Phone:"}</span>
                        <span className="font-medium text-dark">{order.customer_phone}</span>
                      </div>
                    )}
                    {order.customer_email && (
                      <div>
                        <span className="text-dark-5 block">{locale === "ar" ? "البريد الإلكتروني:" : "Email:"}</span>
                        <span className="font-medium text-dark">{order.customer_email}</span>
                      </div>
                    )}
                    {order.customer_street_address && (
                      <div>
                        <span className="text-dark-5 block">{locale === "ar" ? "عنوان التسليم:" : "Delivery Address:"}</span>
            ...
            {order.order_items && order.order_items.length > 0 && (
              <div className="mt-8 pt-8 border-t border-gray-3">
                <h3 className="text-xl font-semibold text-dark mb-4">{locale === "ar" ? "المنتجات المطلوبة" : "Order Items"}</h3>
                <div className="space-y-4">
                  {order.order_items.map(item => (
                    <div key={item.id} className="flex justify-between items-center py-3 border-b border-gray-3">
                      <div>
                        <h4 className="font-medium text-dark">{item.products?.title || `Product ${item.product_id}`}</h4>
            ...
            <div className="mt-8 pt-8 border-gray-3 text-center">
              <h3 className="text-xl font-semibold text-dark mb-4">{locale === "ar" ? "ماذا بعد؟" : "What's Next?"}</h3>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
                <div className="p-4 bg-gray-1 rounded-lg text-center">
                  <div className="text-blue text-2xl mb-2">📞</div>
                  <h4 className="font-medium text-dark mb-2">{locale === "ar" ? "سنتواصل معك" : "We'll Contact You"}</h4>
                  <p className="text-sm text-dark-5">{locale === "ar" ? "سنقوم بالاتصال بك لتأكيد الطلب وترتيب التسليم" : "We'll call you to confirm your order and arrange delivery"}</p>
                </div>
                <div className="p-4 bg-gray-1 rounded-lg text-center">
                  <div className="text-blue text-2xl mb-2">🚚</div>
                  <h4 className="font-medium text-dark mb-2">{locale === "ar" ? "التسليم السريع" : "Fast Delivery"}</h4>
                  <p className="text-sm text-dark-5">{locale === "ar" ? "سيتم توصيل طلبك في أقرب وقت ممكن" : "Your order will be delivered as soon as possible"}</p>
                </div>
              </div>

              <div className="space-x-4">
                <Link href="/" className="ml-4 inline-block bg-blue text-white px-6 py-3 rounded-md hover:bg-blue-dark transition-colors">{locale === "ar" ? "متابعة التسوق" : "Continue Shopping"}</Link>
                <Link href="/contact" className="inline-block border border-blue text-blue px-6 py-3 rounded-md hover:bg-blue hover:text-white transition-colors">{locale === "ar" ? "اتصل بنا" : "Contact Us"}</Link>
              </div>
            </div>
